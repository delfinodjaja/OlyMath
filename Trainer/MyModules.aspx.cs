...
{
    public partial class MyModules : BasePage
    {
        ...
        private void LoadTrainerModules()
        {
            ...
            {
                string sql = @"
                    SELECT m.Id, m.Title, m.Topic, m.Status,
                    (SELECT COUNT(DISTINCT up.UserId) FROM UserProgress up WHERE up.ModuleId = m.Id) AS EnrolledCount
                    FROM Modules m
